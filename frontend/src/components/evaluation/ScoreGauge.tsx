...
import { Verdict, EvaluatorMode } from '../../types';
...
export const ScoreGauge: React.FC<ScoreGaugeProps> = ({ score, verdict, evaluatedBy }) => {
  const getVerdictDetails = (v: Verdict) => {
    switch (v) {
      case 'EXCELLENT':
        return {
  ...
  const getEvaluatorBadge = (mode: EvaluatorMode) => {
    switch (mode) {
      case 'HYBRID':

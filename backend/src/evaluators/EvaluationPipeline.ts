...
export interface EvaluationPipelineOptions {
  aiProviderType?: 'gemini' | 'openai' | 'mock' | 'auto';
  timeoutMs?: number;
  deterministicOnly?: boolean;
}

export class EvaluationPipeline implements IEvaluator {
  public readonly name = 'EvaluationPipeline';
  private deterministicEvaluator: DeterministicEvaluator;
  private aiEvaluator: AIEvaluator | null = null;
  private fallbackEvaluator: FallbackEvaluator;
  ...
  private resolveLLMProvider(preference?: string): ILLMProvider {
  ...
  private mergeIssues(aiIssues: any[], detIssues: any[]): any[] {
